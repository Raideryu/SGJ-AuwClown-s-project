...
public class BaseCharacter : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            // создать евент "дошел до точки назначени€"
        }

        if (_target && agent.remainingDistance <= actionRange) // если расто€ние до цели меньше расто€ни€ действи€
        {
            Action();
        }
    }
    ...
    {
        if (target == this.gameObject) return;// проверка сам на себ€
